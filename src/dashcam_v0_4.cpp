...
//
// v0.4 = v0.3 (UVC passthrough recording + IMX296 lane detection + driver
// drowsiness monitoring) made safe to run UNATTENDED, started by systemd at
// ignition instead of by hand over SSH.  Three differences, all of them about
// what happens when nobody is watching:
//
//   1. RECORDING-ONLY STARTUP.  Lane detection, driver monitoring, the vehicle
//      bridge and network bring-up are gated on the <Startup> config section
//      and all default OFF, so a boot does the one job that must not fail —
//      record the road — and nothing else.  Each is switched back on by editing
//      the config; no rebuild.  This also takes the WiFi associate (up to
//      WifiTimeoutSec) and both TensorRT engine loads off the boot path.
//
//   2. SEGMENTED RECORDING.  Footage is written as fixed-length segments
//      (<Recording><SegmentSeconds>) via splitmuxsink, each with its own .ass
//      sidecar whose timestamps restart at that segment.
//      Matroska already survives an abrupt power cut on its own, so this is
//      about file management rather than rescuing footage: bounded unflushed
//      loss, finalised Cues on every closed segment, bounded file size, and —
//      the one that actually matters unattended — RETENTION.  A full card
//      silently stops recording, and only whole segments can be aged out.
//
//   3. STABLE CAMERA IDENTITY.  A <Camera> <Device> may be a /dev/v4l/by-id/...
//      symlink, resolved to its real node at startup, and <Camera name="record">
//      pins the road camera the way name="cabin" already pinned the driver one.
//      /dev/videoN order is not stable across reboots, and an unattended boot
//      that grabs the cabin camera records a face instead of the road.
//
// Camera roles:
...
//
// NOTE the two-stage gating in v0.4: resolveCameraConfiguration() still assigns
// every role its hardware, and the <Startup> flags then decide which of those
// roles actually starts.  Keeping them separate means the startup log still
// reports the profile the RIG could run ("RECORD + LANES + DRIVER-MONITOR")
// alongside what was switched on, so a camera left idle by config is
// distinguishable from one that was never detected.
//
// Recording-camera selection:
//   - Pinned: a <Camera name="record" type="USB"> entry with a non-empty
//     <Device> reserves that device for recording; it wins over any other USB
//     config entry.  The <Device> may be a stable /dev/v4l/by-id/... symlink.
//   - Auto: the first precompressed-capable USB camera the cabin pin and driver
//     monitor have not claimed.
//
...
//
// Build: `make` (target dashcam_v0_4; needs TRT + OpenCV, no VPI dependency).
// Run:   docker_dev/launchcode_v0_4.sh   (starts this binary immediately)
// Boot:  deploy/install_dashcam_service.sh (systemd unit; starts at ignition)
...
// Resolve a configured <Device> to the real /dev/videoN node it names.
//
// /dev/videoN numbering is assigned in probe order and is NOT stable across
// reboots or replugs, so pinning a camera by node is exactly as unreliable as
// not pinning it at all — which matters far more in v0.4 than it did before,
// because v0.4 starts unattended at boot with nobody watching which camera it
// picked.  udev's /dev/v4l/by-id/ symlinks are keyed on the USB descriptor
// (vendor/product/serial) and DO survive both, so an operator can pin a
// specific physical camera and have it mean the same thing every boot.
//
// Accepts a by-id or by-path symlink and follows it to the node that
// getCameraList() reports.  Anything already a real node, or that cannot be
// resolved (a pin naming an unplugged camera), is returned unchanged so the
// caller's own "pinned camera not found" handling still fires.
static std::string resolveDeviceNode(const std::string& configured) {
    if (configured.empty()) return configured;
    std::error_code ec;
    const fs::path canonical = fs::canonical(fs::path(configured), ec);
    if (ec) return configured;
...
// USB and runs lane detection on the IMX296.  The driver-monitoring camera
...
static std::string describeConfiguration(const CameraConfiguration& c) {
    ...
    if ( r &&  l &&  d) return "RECORD + LANES + DRIVER-MONITOR (all roles filled)";
...
resolveCameraConfiguration(const std::vector<cameraInfo>& cams,
    ...
    // Every configured <Device> passes through resolveDeviceNode() before it is
    // compared with a discovered camera's address, so a stable
    // /dev/v4l/by-id/... symlink pins the same physical camera across reboots.
    auto cfgDevice = [](const dashcam::config::CameraConfig& cfg) {
    ...
    for (const auto& cfg : configs) {
        if (cfg.type != "CSI" || !(bool)cfg.enabled ||
            ...
            continue;
        const cameraInfo* camera =
            findCamera(cfgDevice(cfg), CAMERA_TYPE::CSI);
        if (!camera) {
    ...
    //
    // Two passes, so a <Camera name="record"> entry wins over any other
    // configured USB camera regardless of where it sits in the file.  Naming the
    // road camera explicitly is what an unattended boot needs — with several USB
    // entries present, "whichever is listed first" is intent by accident.  Pass 2
    // keeps the pre-v0.4 behaviour (any enabled non-cabin entry with a device),
    // so existing configs that never named a camera still work.
    for (int pass = 0; pass < 2 && !cc.record; ++pass) {
        for (const auto& cfg : configs) {
            if (cfg.type != "USB" || cfg.name == "cabin" || !(bool)cfg.enabled ||
                std::string(cfg.device).empty())
                continue;
            if ((pass == 0) != (cfg.name == "record")) continue;
            const cameraInfo* camera =
                findCamera(cfgDevice(cfg), CAMERA_TYPE::USB);
            if (!camera ||
                (!cabinDev.empty() && camera->address == cabinDev) ||
                (cc.driver && camera->address == cc.driver->address)) {
                // A pin that names a camera which is absent, or already claimed
                // by the cabin role, must not silently fall through to some
                // other camera: recording the wrong one unattended is worse than
                // the operator being told their pin did not take.
                if (cfg.name == "record")
                    log(LogLevel::ERROR,
                        "pinned recording camera " + (std::string)cfg.device +
                        " not available — falling back to auto-selection");
                continue;
            }
            ...
            if (cfg.name == "record")
                log(LogLevel::ERROR,
                    "pinned recording camera " + camera->address +
                    " has no usable precompressed (MJPEG/H264) format — "
...
// invariants.  Run with:
//   dashcam_v0_4 --self-test-camera-configuration
...
static int runCameraConfigurationSelfTest() {
    ...
    auto usbConfig = [](const std::string& name, const std::string& address,
                        bool enabled = true) {
        dashcam::config::CameraConfig c;
        c.name = name;
        c.type = "USB";
    ...
    // ── v0.4: recording-camera pinning ───────────────────────────────────────

    // A name="record" pin wins over another enabled USB entry listed FIRST.
    // Without the two-pass order this test records /dev/other, i.e. whichever
    // camera happened to be written higher up the config file.
    {
        const std::vector<cameraInfo> cams{
            camera(CAMERA_TYPE::USB, "/dev/other",
                   {format(V4L2_PIX_FMT_MJPEG)}),
            camera(CAMERA_TYPE::USB, "/dev/road",
                   {format(V4L2_PIX_FMT_MJPEG)})
        };
        const std::vector<dashcam::config::CameraConfig> configs{
            usbConfig("spare",  "/dev/other"),
            usbConfig("record", "/dev/road")
        };
        const auto cc = resolveCameraConfiguration(cams, configs, noLog);
        check(cc.record && cc.record->address == "/dev/road",
              "name=\"record\" pin outranks an earlier USB config entry");
    }

    // A record pin and a cabin pin must be honoured together, each taking the
    // device it names — the unattended-boot case this whole mechanism exists for.
    {
        const std::vector<cameraInfo> cams{
            camera(CAMERA_TYPE::USB, "/dev/cabin",
        ...
        const auto cc = resolveCameraConfiguration(cams, configs, noLog);
        check(cc.record && cc.record->address == "/dev/road" &&
              cc.driver && cc.driver->address == "/dev/cabin" && cc.driverPinned,
              "record and cabin pins are honoured together");
    }

    // A record pin naming an absent device must fall back to auto-selection
    // rather than leaving the rig with no recording at all.
    {
        const std::vector<cameraInfo> cams{
            camera(CAMERA_TYPE::USB, "/dev/road",
                   {format(V4L2_PIX_FMT_MJPEG)})
        ...
        const std::vector<dashcam::config::CameraConfig> configs{
            usbConfig("record", "/dev/not-plugged-in")
        };
        const auto cc = resolveCameraConfiguration(cams, configs, noLog);
        check(cc.record && cc.record->address == "/dev/road",
              "record pin naming an absent device falls back to auto-selection");
    }

    // An explicitly disabled record entry must not be selected.
    {
        const std::vector<cameraInfo> cams{
            camera(CAMERA_TYPE::USB, "/dev/road",
                   {format(V4L2_PIX_FMT_MJPEG)}),
            camera(CAMERA_TYPE::USB, "/dev/spare",
                   {format(V4L2_PIX_FMT_MJPEG)})
        };
        const std::vector<dashcam::config::CameraConfig> configs{
            usbConfig("record", "/dev/road", /*enabled=*/false)
        };
        const auto cc = resolveCameraConfiguration(cams, configs, noLog);
        check(cc.record && cc.record->address == "/dev/spare",
              "disabled record pin is skipped, not forced");
    }

    // ── v0.4: by-id symlink resolution ───────────────────────────────────────
    // The point of the whole pinning exercise: a config that names a camera by
    // its stable udev symlink must resolve to the /dev/videoN node discovery
    // reports.  Uses a real symlink in a temp dir so the resolver's actual
    // filesystem behaviour is exercised, not a mock of it.
    {
        std::error_code ec;
        const fs::path dir =
            fs::temp_directory_path(ec) / "dashcam_selftest_byid";
        fs::remove_all(dir, ec);
        fs::create_directories(dir, ec);
        const fs::path node = dir / "video9";
        const fs::path link = dir / "usb-Acme_Dashcam_1234-video-index0";
        { std::ofstream touch(node.string()); }
        fs::create_symlink(node, link, ec);

        if (ec) {
            check(false, "by-id self-test could not create its symlink fixture");
        } else {
            const std::vector<cameraInfo> cams{
                camera(CAMERA_TYPE::USB, node.string(),
                       {format(V4L2_PIX_FMT_MJPEG)}),
                camera(CAMERA_TYPE::USB, "/dev/other",
                       {format(V4L2_PIX_FMT_MJPEG)})
            };
            const std::vector<dashcam::config::CameraConfig> configs{
                usbConfig("record", link.string())
            };
            const auto cc = resolveCameraConfiguration(cams, configs, noLog);
            check(cc.record && cc.record->address == node.string(),
                  "by-id symlink in <Device> resolves to the real node");
        }
        fs::remove_all(dir, ec);
    }
...
int main(int argc, char* argv[]) {
    ...
    //
    // v0.4 gates this on <Startup><NetworkBringUp>, which defaults OFF: the
    // associate blocks for up to WifiTimeoutSec (20 s by default), and a car
    // that starts away from its known SSID pays that on every ignition before
    // the first frame is recorded.
    const bool networkAtBoot = (bool)cfg.startup.networkBringUp;
    bool offline = !networkAtBoot;
    std::string wifiSummary;
    {
        dashcam::network::WifiConnectConfig wc;
        wc.enabled         = networkAtBoot && (bool)cfg.network.wifiConnectEnabled;
        ...
        if (!networkAtBoot) {
            wifiSummary = "network: not started (Startup/NetworkBringUp is off)";
        } else if (wc.enabled) {
    ...
    const std::string timeSyncSummary =
        !networkAtBoot
            ? std::string("time sync: not started (Startup/NetworkBringUp is off)")
    ...
    log(dashcam::log::LogLevel::INFO,
        "dashcam v0.4 starting (recording-first; subsystems gated by <Startup>)");
    ...
    // ── v0.4 startup gate ────────────────────────────────────────────────────
    // v0.4 runs unattended from a systemd unit at boot, where the one job that
    // must never fail is recording the road.  Lane detection and driver
    // monitoring are optional extras that each cost a TensorRT engine load
    // (seconds) and steady CPU on a box with no NVENC, so they default OFF and
    // are switched on per-drive from the config.
    //
    // Clearing the role pointers here gates the engine load, the camera start,
    // every tee branch and the main loop's result handling in ONE place —
    // rather than an `if` at each of those sites, where a missed one would
    // start hardware the operator asked to stay off.  Downstream code already
    // treats a null role as "this role has no camera", which is exactly the
    // state we want.
    if (laneInfo && !(bool)cfg.startup.laneDetection) {
        log(dashcam::log::LogLevel::INFO,
            "lane detection OFF at startup (Startup/LaneDetection); "
            "IMX296 " + laneInfo->address + " left idle");
        laneInfo = nullptr;
    }
    if (drvInfo && !(bool)cfg.startup.driverMonitoring) {
        log(dashcam::log::LogLevel::INFO,
            "driver monitoring OFF at startup (Startup/DriverMonitoring); "
            "cabin camera " + drvInfo->address + " left idle");
        drvInfo = nullptr;
    }
    ...
    //
    // v0.4 gates it on <Startup><VehicleBridge>, off by default: when it is not
    // running the sidecar simply renders dashes for the fields it would supply,
    // which is the correct rendering for "no live source" and costs the
    // recording nothing.  Leaving it off also keeps a retrying RX thread and a
    // USB device open off the boot path.
    dashcam::commlink::CommLink bridge;
    const bool bridgeEnabled = (bool)cfg.startup.vehicleBridge;
    if (!bridgeEnabled) {
        log(dashcam::log::LogLevel::INFO,
            "telemetry bridge OFF at startup (Startup/VehicleBridge); "
            "GPS/IMU overlay fields will render as dashes");
    } else {
    ...
    if (usbInfo) {
        ...
        // Segmented (see <Recording><SegmentSeconds>): bounded file size and
        // age-able footage, which is what an unattended card needs.
        // eosTimeoutMs is passed explicitly only because segmentSeconds
        // follows it positionally.
        recActive = rec.startRecording(
            usbInfo->address, rfmt, recFile,
            (uint32_t)(int)cfg.recording.recordFps,
            /*eosTimeoutMs=*/4000,
            (uint32_t)(int)cfg.recording.segmentSeconds);
        if (!recActive) {
            log(dashcam::log::LogLevel::ERROR,
    ...
    // Name what will actually appear on disk: in segmented mode recFile is only
    // a base name and no such file is ever written.
    const std::string recTarget =
        (int)cfg.recording.segmentSeconds > 0
            ? dashcam::record::Recorder::segmentPattern(recFile)
            : recFile;
    log(dashcam::log::LogLevel::INFO,
        std::string(recActive ? "recording: " + recTarget : "recording OFF")
    ...
    if (recActive) {
        auto sizeKb = [](const fs::path& f) -> long {
            std::error_code ec;
            const auto n = fs::file_size(f, ec);
            return ec ? 0L : (long)(n / 1024);
        };
        if ((int)cfg.recording.segmentSeconds > 0) {
            // Segmented: sum every file this run produced.  They share the
            // timestamped stem of recFile, so match on that prefix rather than
            // trying to stat a single name that was never written.
            const fs::path stem = fs::path(recFile).stem();   // primary_<ts>
            const fs::path dir  = fs::path(recFile).parent_path();
            long videoKb = 0, assKb = 0;
            int  segments = 0;
            std::error_code ec;
            for (const auto& e : fs::directory_iterator(dir, ec)) {
                const std::string name = e.path().filename().string();
                if (name.rfind(stem.string() + "_", 0) != 0) continue;
                if (e.path().extension() == ".mkv") { videoKb += sizeKb(e.path()); ++segments; }
                else if (e.path().extension() == ".ass") assKb += sizeKb(e.path());
            }
            log(dashcam::log::LogLevel::INFO,
                "stopped. " + std::to_string(segments) + " segment(s), primary "
                + std::to_string(videoKb) + " KB, telemetry "
                + std::to_string(assKb) + " KB");
        } else {
