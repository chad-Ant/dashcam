...
/*
 * DASHCAM PATCH: vendoring marker.
 *
 * Consumers #error on its absence, so a build that resolves <BNO055.h> from the
 * global Arduino libraries folder fails loudly instead of silently linking a
 * copy nothing version-checks. The global copy also ships BNO055_support.cpp,
 * which drives the bus with raw Wire calls and discards every return value -
 * the exact class of fault that cost this project a week of diagnosis.
 *
 * See vendor/BNO055/PATCHES.md.
 */
...
/*
 * DASHCAM PATCH: C linkage guards.
 *
 * BNO055.c is C and this header shipped without them, so every declaration got
 * C++ mangling when included from a .cpp or an .ino while the definitions kept
 * C names. Everything compiled and then failed at link with "undefined
 * reference to bno055_init(bno055_t*)" - note the argument list, which is the
 * tell that the caller looked for a mangled symbol.
 *
 * Upstream never hit this because its own glue was the only consumer and the
 * examples reached the driver through that. This project calls it directly.
 */
#ifdef __cplusplus
extern "C" {
#endif
