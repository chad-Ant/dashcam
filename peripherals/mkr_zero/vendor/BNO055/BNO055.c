...
/* DASHCAM PATCH: silence two upstream warning classes for THIS FILE ONLY.
 *
 * The project builds at --warnings all, and unpatched this one file emits 307
 * warnings: 235 -Wpointer-compare and 72 -Wmisleading-indentation. That is not
 * a cosmetic complaint. It is 2000 lines of console output per build, and a
 * genuine warning in this project's own code lands in the middle of it and is
 * never seen - which defeats the entire reason --warnings all is on.
 *
 * Both classes were inspected before being silenced, and both are benign:
 *
 *   -Wpointer-compare        `if (p_bno055 == BNO055_Zero_U8X)`, where that
 *                            macro is `(unsigned char)0`. In C an integer
 *                            constant expression of value 0 IS a null pointer
 *                            constant, so this is a correct null check written
 *                            in a way GCC suspects is a typo for *p_bno055.
 *
 *   -Wmisleading-indentation The setters indent an UNGUARDED `if (status ==
 *                            SUCCESS)` as though the preceding one-line `if`
 *                            covered it. Running it unconditionally is what the
 *                            code wants; only the whitespace lies.
 *
 * Scoped to this translation unit, so the project's own sources keep every
 * warning. Delete these three pragmas to see the upstream output again.
 */
...
BNO055_RETURN_FUNCTION_TYPE bno055_init(struct bno055_t *bno055)
	{
		...
		p_bno055 = bno055;
		/*
		 * DASHCAM PATCH: honour a caller-supplied address.
		 *
		 * Upstream assigned BNO055_I2C_ADDR unconditionally, silently discarding
		 * whatever the caller had put in dev_addr. That default is 0x28 - which
		 * the datasheet calls the ALTERNATIVE address; the default is 0x29, and
		 * GY breakouts commonly land there because COM3 is left floating. So on
		 * this hardware every read in this function went to an address with
		 * nothing on it, chip_id came back as whatever the failed read left in
		 * the buffer, and the scan that found the real device was overwritten
		 * one line into init.
		 *
		 * Zero still means "caller did not choose", so upstream behaviour is
