...
- **Multi-tasking the stream**: the IMX296 is inference-primary (the main dashcam recording is handled by a separate USB camera). One 60 fps capture feeds multiple AI inference workloads at different rates (lane detection at 30/60 fps, sign reading at 5 fps, etc.) via `tee` + `videorate` or DeepStream's `nvinfer interval=N`. A **debug video branch** should also be tee'd off — software-encode to a low-priority file or ring buffer so you can review what the inference cameras saw during development and post-incident analysis. Keep the debug encode cheap (low bitrate / lower fps, e.g. `videorate ! … framerate=15/1 ! x264enc tune=zerolatency speed-preset=ultrafast bitrate=2000`) so it doesn't compete with the primary USB-camera recording for CPU. See `deepstream-pipelines.md`.
